...
export default {
  setup() {
    ...
    const picks = usePicks({
      fetchData: () => get(getSiteData + "?location=data/berks-frontpage.json"),
      saveData: (data) =>
        post(postSiteData + "?location=data/berks-frontpage.json", data),
    });
...
<template>
  <MetaHead>
    <title>Berks Frontpage Editor • Spotlight PA Almanack</title>
  </MetaHead>
  <div>
    <BulmaBreadcrumbs
      :links="[
        ...
        {
          name: 'Berks Frontpage Editor',
          to: { name: 'berks-editor' },
        },
      ]"
    ></BulmaBreadcrumbs>

    <h1 class="title">Berks Frontpage Editor</h1>
