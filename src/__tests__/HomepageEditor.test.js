...
describe("HomepageEditor", () => {
  ...
  it("push function adds article to featuredStories via event", async () => {
    ...
    await pageFinder.vm.$emit("select-page", {
      filePath: "/content/news/test-article.md",
    });

    expect(editorsPicks.featuredStories).toContain(
      "/content/news/test-article.md"
    );
  });
  ...
  it("multiple articles can be added", async () => {
    ...
    const pageFinder = wrapper.findComponent({ name: "PageFinder" });
    await pageFinder.vm.$emit("select-page", {
      filePath: "/content/news/article1.md",
    });
    await pageFinder.vm.$emit("select-page", {
      filePath: "/content/news/article2.md",
    });
