...
func TestIsUniquenessViolation(t *testing.T) {
	...
	{ // No errors to insert some key
		...
		be.NilErr(t, err)
		be.False(t, dbutils.IsUniquenessViolation(err, ""))
		be.False(t, dbutils.IsUniquenessViolation(err, "blah"))
		be.False(t, dbutils.IsUniquenessViolation(err, "option_key_key"))
	}
	{ // Get option_key_key uniqueness errors on repeat insertions of the same key
		_, err := dbtx.Exec(t.Context(), "insert into option(key, value) values ('k', 'v')")
		be.Nonzero(t, err)
		be.True(t, dbutils.IsUniquenessViolation(err, ""))
		be.False(t, dbutils.IsUniquenessViolation(err, "blah"))
		be.True(t, dbutils.IsUniquenessViolation(err, "option_key_key"))
	}
}
